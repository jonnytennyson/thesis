...
\subsection{Illumina Sequencing Technology}
Sequencing is the determination of the sequence of base pairs in a strand of DNA. Illumina sequencing exploits the complementary base pairing behaviour of DNA bases to identify the sequence of bases, a single base at a time via ``sequencing by synthesis". The technology uses reversible terminator chemistry to sequence short strands of DNA by cycling through a series of elongation, fluorescence imaging and termiator removal steps (Fig.~\ref{fig:illumina}.  Although this is still a fluorescence based technique, there are several major differences this sequencing technique and the single molecule confocal experimentes described in the previous chapters. Firstly,  However, unlike in the smFRET experiments described in previous chapters, instead of attempting to distinguish individual fluorophores against a noisy background signal, the Illumina sequencing process involves a ``clustering" step, which replicates the DNA strands within a small area, allowing the signal to be amplified.  

\subsection{De Novo Sequence Assembly}
\subsection{Mate Pair Sequencing}
