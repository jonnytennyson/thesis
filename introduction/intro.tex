...
The fluorescence excitation-emission cycle involves transitions between the singlet ground state ($\text{S}_0$) and first excited state ($\text{S}_1$) of the fluorophore. However, other competing, non-radiative processes can occur. The most significant of these is inter-system crossing, involving a non-radiative transition from a singlet excited state to a triplet excited state. Internal conversion -- a non-radiative transition to a lower singlet state -- is also possible.

One useful parameter for characterising a fluorophore is its quantum yield, $\phi_f$, defined as: 
...
where $k_f$ and $k_{nr}$ are the rates of fluorescence emission and non-radiative emission respectively.

The typical timescale for a single fluorescence excitation-emission cycle is approximately $10^{-10} - 10^{-7}$ s. This timescale is dominated by the lifetime of the excited state, (typically $10^{-10} - 10^{-7}$ s), whilst excitation and emission processes occur on a timescale that is several orders of magniude shorter (absorption $10^{-15}$ s, relaxation $10^{-12} - 10^{-10}$ s). Intersystem crossing ($10^{-10} - 10^{-8}$ s) and internal conversion ($10^{-11} - 10^{-9}$ s) are slower processes.
