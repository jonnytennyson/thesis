...
Fluorescence Correlation Spectroscopy (FCS)~\cite{Magde1972} is an extension of the Confocal method described above. In FCS, a higher concentration ($1$ nM - $100$ nM) of analyte solution is used, such that multiple fluorophores occupy the confocal volume an any time. Temporal fluctuations in the detected fluorescence emission can be used to determine physical parameters of the analyte solution, such as molecular concentrations and diffusion constants. These are determined by fitting a fluorescence autocorrelation function (Eq.~\ref{eq:fcs}), 
